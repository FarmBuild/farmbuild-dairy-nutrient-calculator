...
//        path = path.substring(0, path.indexOf('?'));
   ...
   function drawSpecificChart(farmData, what_to_draw, imp_or_exp){
        ...
        var datafield;
		var milkdatafield;
        var imp_exp_str="Import";

        if(imp_or_exp == "export") imp_exp_str="Export";
        }catch(Err){
            alert(Err.message);
        }
		var totalfarmarea = parseFloat(farmData.area);//parseFloat(unescape(params['totalfarmarea'])); //needs to get area

		var milkingarea = parseFloat(farmData.nutrientCalculator.summary.milkingAreaInHa);
        //var data_export = new google.visualization.DataTable();
        ...
        switch(what_to_draw){
            case 'mass':
                ...
                datalabel='Mass';
				fixation_val = parseFloat(eval("farmData.nutrientCalculator.legumes.availableNitrogenFromLegumesPerHaInKg/1000"))*milkingarea;
				atmosphere_val=parseFloat(totalfarmarea*parseFloat(eval('3/1000')));
                break;
            case 'nitrogen':
                milkdatafield =datafield='nitrogenInKg';
                datalabel='Nitrogen';
				fixation_val = parseFloat(eval("farmData.nutrientCalculator.legumes.availableNitrogenFromLegumesPerHaInKg"))*milkingarea;
